...
export default function SaveDestination() {
  ...
  const [userSavedDestinationItem, setUserSavedDestinationItem] = useState<
    UserSavedDestinationType[]
  >([]);
    ...
    if (id && token) {
      try {
        const response = await axios.get(
          `http://localhost:4040/api/v1/user-saved-destination/${id}`,
          {
            headers: {
  ...
  const currentRecords = userSavedDestinationItem.slice(
    indexOfFirstRecord,
    indexOfLastRecord
  );
  const totalPages = Math.ceil(
    userSavedDestinationItem.length / recordsPerPage
  );
          ...
          {currentRecords.map((userSavedDestination) => (
            <div key={userSavedDestination.id} css={s.card}>
                ...
                <img
                  ...
                  onClick={() =>
                    navigateToDestinationDetail(
                      userSavedDestination.destinationId
                    )
                  }
                />
              </div>
              <div css={s.cardContent}>
                ...
                <p css={s.info}>
                  {userSavedDestination.location} |{" "}
                  {userSavedDestination.address}
                  <button
                ...
                <p css={s.hours}>
                  {userSavedDestination.openingHours} ~{" "}
                  {userSavedDestination.closedHours}
                </p>
                <p css={s.phone}>☎ {userSavedDestination.phoneNumber}</p>
                <p css={s.rating}>
                  ⭐ {userSavedDestination.rating.toFixed(1)}
                </p>
                <button
                  onClick={() => handleDelete(userSavedDestination.id)}
                  css={s.deleteButton}
                >
                  <FaTrash />
                </button>
