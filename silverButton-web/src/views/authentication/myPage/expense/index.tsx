...
const Calculator = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (token) {
      try {
        const response = await axios.post(`http://localhost:4040/api/v1/expense/`, newExpenses, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleDelete = async (expenseId: number) => {
    ...
    if (token) {
      try {
        await axios.delete(`http://localhost:4040/api/v1/expense/${expenseId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const currentRecords = expensItem.slice(indexOfFirstRecord, indexOfLastRecord);
  ...
  const [isVisible, setIsVisible] = useState<boolean>(false); // 애니메이션을 위한 상태
  ...
  지출 금액: {expense.amount.toLocaleString()}원
        ...
        <div
          css={[
            s.calculatorStyle,
            isVisible ? s.showCalculator : s.hideCalculator, // 애니메이션 효과 추가
          ]}
        >
