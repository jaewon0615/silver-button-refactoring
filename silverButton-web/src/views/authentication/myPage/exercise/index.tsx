...
export default function Exercise() {
  ...
  const handleDelete = async (exerciseId: number) => {
    ...
    if (token) {
      try {
        await axios.delete(
          `http://localhost:4040/api/v1/exercise/${exerciseId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const currentRecords = exerciseItem.slice(
    indexOfFirstRecord,
    indexOfLastRecord
  );
              ...
              <div key={exercise.id} css={s.recordItem}>
                <div>
                  <h3 css={s.datePageText}>
                    운동 날짜:{" "}
                    {new Date(exercise.exerciseDate).toLocaleString("ko-KR", {
                      year: "numeric",
                      month: "2-digit",
                      day: "2-digit",
                    })}
