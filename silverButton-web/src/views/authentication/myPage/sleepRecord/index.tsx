...
export default function SleepRecord() {
  ...
  const fetchSleepRecord = async () => {
    ...
    if (id && token){
      try{
        const response = await axios.get(`http://localhost:4040/api/v1/sleep-record/${id}`,{
          headers:{
  ...
  if(token){
    try{
      const response = await axios.post(`http://localhost:4040/api/v1/sleep-record/`,newRecord,{
        headers:{
          Authorization:`Bearer ${token}`,
        },
      }
    ...
    setSleepRecordItem((prev) => [response.data.data, ...prev].sort((a,b) => b.createdAt - a.createdAt)
  );
...
const handleDelete = async (recordId: number) => {
  ...
  if(token){
    try{
      await axios.delete(`http://localhost:4040/api/v1/sleep-record/${recordId}`,{
        headers:{
          Authorization:`Bearer ${token}`,
        },
      });

      setSleepRecordItem((prev) => prev.filter((record) => record.id !== recordId)
    );
...
const currentRecords = sleepRecordItem.slice(indexOfFirstRecord, indexOfLastRecord);
...
const formatTime = (time: number) => {
  const timeString = time.toString().padStart(4, '0'); // 4자리 문자열 유지
    ...
    <div css={s.container}>
      <div css={s.conttSt}>
        ...
        <div css={s.recordContainer}>
            ...
            currentRecords.map((sleepRecord) => (
              <div key={sleepRecord.id} css={s.recordItem}>
                ...
                <h3 css={s.datePageText}><FaCalendarAlt css={s.icon} />
                  수면 날짜: {new Date(sleepRecord.sleepDate).toLocaleString("ko-KR", {
                    year: "numeric",
                    month: "2-digit",
                    day: "2-digit",
                  })}
                </h3>
                <h3 css={s.resultPageText}>잠 든 시각:{formatTime(sleepRecord.sleepTime)}분</h3>
                <h3 css={s.resultPageText}>기상 시각:{formatTime(sleepRecord.wakeTime)}분</h3>
                <h3 css={s.resultPageText}>총 수면 시간:{sleepRecord.sleepDuration}시간</h3>
                <h3 css={s.resultPageText}>수면의 질:{sleepRecord.sleepQuality}점</h3>
                <h3 css={s.resultPageText}>잠에서 깬 횟수:{sleepRecord.sleepInterruptionCount}회</h3>
                <h3 css={s.resultPageText}>꿈 내용/여부:{sleepRecord.dreamOccurred}</h3>
                <h3 css={s.resultPageText}>추가 메모{sleepRecord.notes}</h3>
                <p css={s.resultPageText}><strong>제출 날짜:</strong>{new Date(sleepRecord.createdAt).toLocaleDateString()}</p>
                <div>
                  <button onClick={() => handleDelete(sleepRecord.id)} css={s.deleteButton}> <FaTrash /></button>
                </div>
                </div>
              </div>
            ))
          ...
          <button onClick={handleGraphView} css={s.chartButton}>수면 그래프 보기</button>
          {totalPages > 1 && (
